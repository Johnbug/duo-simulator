...
export function DuoModel(props: Props) {
  const copy = getTranslations(props.locale);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const key = new THREE.DirectionalLight(0xfff9ec, 3.5);
      key.position.set(-10, 14, 20);
      scene.add(key);
      const fill = new THREE.DirectionalLight(0xc6dfff, 1.7);
      ...
      const names = {
        landscape: 'QcYncVIkzdbhIbJ',
        portrait: 'iFSuhcVrhLhMisa',
        laptop: 'vHeRSOZUeAIaYwi',
        closed: 'KwwxLhAVqcKFMjD',
        tent: 'qAIPDzqFGVVDXZp',
      };
      const resize = () => {
        ...
        const distance = Math.max(
          29,
          19 / (2 * Math.tan(THREE.MathUtils.degToRad(16)) * camera.aspect),
        );
      ...
      const [gltf, loadedScreens] = await Promise.all([
        ...
        Promise.all(
          Object.entries(names).map(async ([name, file]) => {
            const texture = await textureLoader.loadAsync(
              `/assets/model/${file}.avif`,
            );
      ...
      root = gltf.scene;
      loadedScreens.forEach(([name, texture]) => {
        screenTextures[name] = texture;
      ...
      root.traverse((object) => {
        ...
        (Array.isArray(mesh.material)
          ...
          if (mesh.name.includes('outerDisplayScreenTexture'))
            screenOuter = material;
      ...
      const updatePose = () => {
        ...
        const time = Math.min(
          ...
          (displayAngle / 180) * clip.duration,
        );
        if (Math.abs(time - lastPoseTime) > 0.000001) {
          ...
          root!.traverse((object) => {
            if ((object as import('three').SkinnedMesh).isSkinnedMesh)
              (object as import('three').SkinnedMesh).computeBoundingBox();
          });
          centered.position.set(0, 0, 0);
          presentation.rotation.set(0, 0, 0);
          presentation.updateMatrixWorld(true);
        ...
        if (lastFinish !== current.finish) {
          materials.forEach((material) => {
            ...
            if (
              current.finish === 'night' &&
              !material.map &&
              material !== screenInner &&
              material !== screenOuter &&
              original.r > 0.35 &&
        ...
        if (screenInner) {
          ...
          screenInner.emissive.set(0xffffff);
          screenInner.emissiveIntensity = 1;
          screenInner.toneMapped = false;
        }
        if (screenOuter) {
          screenOuter.emissiveMap =
            screenTextures[current.pose === 'tent' ? 'tent' : 'closed'];
          screenOuter.emissive.set(0xffffff);
          screenOuter.emissiveIntensity = 1;
          screenOuter.toneMapped = false;
        }
        const z = current.portrait
          ? -Math.PI / 2
          : current.pose === 'tent' || current.pose === 'laptop'
            ? Math.PI / 2
            : 0;
      ...
      const animate = (time: number) => {
        ...
        displayAngle = current.dragging
          ? current.angle
          : THREE.MathUtils.lerp(displayAngle, current.angle, smooth);
        if (Math.abs(displayAngle - current.angle) < 0.03)
      ...
      Object.assign(current, latest.current);
      updatePose();
      renderer.render(scene, camera);
      setStatus('ready');
      frame = requestAnimationFrame(animate);
    ...
    load().catch((error) => {
      if (!disposed) {
        console.warn('Duo model could not load', error);
        setStatus('error');
      }
    });
    return () => {
  ...
  return (
    <div className="model-surface" ref={host} data-model-status={status}>
      {status !== 'ready' && (
        <div className="model-loading">
          <img src="/assets/star-white.webp" alt="Apple iPhone Duo" />
          {status === 'loading' ? (
            <p>
              <span className="model-spinner" /> {copy.loadingModel}
            </p>
          ) : (
            <>
              <p>{copy.modelUnavailable}</p>
              <button onClick={props.onFallback}>{copy.continueApps}</button>
            </>
          )}
        </div>
      )}
